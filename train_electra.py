...
from transformers import BertTokenizer, PretrainedConfig, ElectraTokenizer, ElectraModel
...
# TRAIN_BATCH_SIZE = 18
# accumulation_steps = 32 // TRAIN_BATCH_SIZE
# MODEL = 'google/electra-base-discriminator'
# D_MODEL = 768
# SAVE_HEAD = 'saved/electra_base_'
# MODE = 'electra_base_'
# LR = 5e-5
...
D_MODEL = 1024
SAVE_HEAD = 'saved/distance1_electra_'
MODE = 'distance1_electra_'
LR = 2e-6
...
class TweetModel(transformers.BertPreTrainedModel):
    ...
    def __init__(self, conf):
        ...
        self.bert = ElectraModel.from_pretrained(MODEL, output_hidden_states=True, output_attentions=True)
...
def loss_fn(start_logits, end_logits, start_positions, end_positions):
    c_loss = cs_loss(start_logits, end_logits, start_positions, end_positions)
    # use distance loss computed by jaccard score
    d_loss = distance_loss(start_logits, end_logits, start_positions, end_positions)
    loss = (c_loss + d_loss) / 2
    # loss = d_loss
    return loss


def cs_loss(start_logits, end_logits, start_positions, end_positions):
    """
...
def jcd_score(start_positions, end_positions):
...
def distance_loss(start_logits, end_logits, start_positions, end_positions, temperature=0):
    start_score, end_score = jcd_score(start_positions, end_positions)
...
def train_fn(data_loader, model, optimizer, device, scheduler=None):
    ...
    for bi, d in enumerate(tk0):
        ...
        if (bi + 1) % accumulation_steps == 0 or (bi + 1) == len(data_loader):
...
def run(fold):
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=int(len(df_train) / TRAIN_BATCH_SIZE / accumulation_steps * num_warmup_rates),
