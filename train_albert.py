...
from transformers import BertTokenizer, PretrainedConfig, AlbertTokenizer, AlbertForQuestionAnswering, AlbertModel
...
def process_data(tweet, selected_text, sentiment, tokenizer, max_len):
    ...
    selected_text = selected_text.replace('ï¿½', 'i').replace('ï', 'i').replace('``', '"').replace('\'', '"').replace(
        '`', '\'')
    ...
    tweet_offsets = get_offsets(tweet, tokenizer.convert_ids_to_tokens(input_ids)[1:-1], ignore=2)
...
class TweetModel(transformers.BertPreTrainedModel):
    ...
    def __init__(self, conf):
        ...
        self.bert = AlbertModel.from_pretrained(MODEL, output_hidden_states=True, output_attentions=True)
