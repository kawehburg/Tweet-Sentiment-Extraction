...
class TweetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['origin_str'] = origin_str  # 写入data
    ...
    def get_target_idx(self, row, tweet, offsets, origin_str):
        ...
        # len_twi = len(tweet)
        # spc_targets = [0] * len(tweet)  # 这个列表就是原来有多个空格，现在就剩下一个空格了，那么空格在哪里少了多少，标志
        # i = 0
        # j = 0
        # while True:
        #     if j >= len_twi:
        ...
        # 这里就是加多少空格的偏移对标，没有用处的，分析用
        # spc_cnt = 0
        # for i in range(len(tweet)):
        #     if char_targets[i] == 1:  # 一旦发现gold label，马上停止计数空格
        #         break
        #
        #     if spc_targets[i] != 0:  # 其他情况下就统计空格数量
...
def get_selected_text(text, start_idx, end_idx, offsets, origin_text):
    selected_text = ""
    reg_rule = re.compile(r"[ï¿½â]")  # 处理奇葩中文字符{茂驴陆}，暴力
    reg_all = reg_rule.findall(text)
    
    start_index = -1  # 因为要往前补，所以记录一下最前端位置
    end_index = -1
    # 如果正则没有发现这个奇葩字符，就不管他，按照原来计划进行
    if len(reg_all) != 0:
        tok_new = bptokenizer.encode(text)  # 这里要重新算一下token
        # 重新计算偏移量，所以需要按照之前的规定补4长度
        new_offset = []
        for _ in range(4):
        ...
        asa = len(text)
        # 下面就是按照原来的方式去计算select_text
        for ix in range(start_idx, end_idx + 1):
            if ix > start_idx:
                if new_offset[ix] == new_offset[ix - 1]:  # 这里是因为"Â"抄了之前的标志，二者取其一就行，直接continue
                    continue
                else:
        ...
        for ix in range(start_idx, end_idx + 1):
            if start_index == -1:  # 第一次出现，备份初始位置
                start_index = offsets[ix][0]
            selected_text += text[offsets[ix][0]: offsets[ix][1]]
            if (ix + 1) < len(offsets) and offsets[ix][1] < offsets[ix + 1][0]:
                selected_text += " "
    
    # ----发现可能有start_index逻辑错误，注意检查-----
    # 下面是处理空格偏移量
    space_cnt = 0
    space_flg = False
    for i in range(len(origin_text)):
        if i > start_index:  # 只需要看初始位置之前出现了多少次空格
            break
        if origin_text[i] == " ":
            if space_flg == False:
                space_flg = True
    ...
    if space_cnt > 1:
        if start_index - space_cnt > 0:
            st_idx = start_index - space_cnt  # 计算初始位置
            selen = len(selected_text)  # 计算文本长度
            ed_idx = selen + st_idx  # 计算最终位置
            selected_text = origin_text[st_idx:ed_idx]  # 在原文本里面裁剪
        # print(123)
...
def compute_jaccard_score(text, start_idx, end_idx, start_logits, end_logits, offsets, origin_text):
    ...
    if start_pred > end_pred:
        ...
        pred = get_selected_text(text, start_pred, end_pred, offsets, origin_text)  # 加参数
    
    true = get_selected_text(text, start_idx, end_idx, offsets, origin_text)  # 加参数
...
def train_model(model, dataloaders_dict, criterion, optimizer, num_epochs, filename):
    ...
    for epoch in range(num_epochs):
        for phase in ['train', 'val']:
            ...
            tk0 = tqdm(dataloaders_dict[phase], total=len(dataloaders_dict[phase]), ncols=80)
            for data in tk0:
                ids = data['ids'].cuda()
                ...
                origin_text = data["origin_str"]  # 加参数
            ...
            if epoch == 2:
                ...
                last_jcd = epoch_jaccard
                
            if epoch == 3 and epoch_jaccard > last_jcd:
                torch.save(model.state_dict(), filename)
